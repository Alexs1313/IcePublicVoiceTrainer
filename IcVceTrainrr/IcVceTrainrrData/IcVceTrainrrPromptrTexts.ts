...
export const icVceTrainrrPROMPTR_CATEGORIES: PromptrCategory[] = [
  {
    id: 'public-speaking',
...
Object.values(PROMPTR_TEXTS).forEach(texts => {
  texts.forEach(item => {
    item.preview = icVceTrainrrPreview(item.body);
  });
});

export const getPromptrCategory = (id: PromptrCategoryId) =>
  icVceTrainrrPROMPTR_CATEGORIES.find(c => c.id === id);

/** @deprecated Use icVceTrainrrGetLibraryTexts from IcVceTrainrrTextLibraryStore */
