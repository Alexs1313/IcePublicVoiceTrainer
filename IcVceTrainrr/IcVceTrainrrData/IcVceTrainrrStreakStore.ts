...
const icVceTrainrrSTORAGE_KEY = '@ice_voice_app_visits_v1';
...
const icVceTrainrrLoadVisitDates = async (): Promise<string[]> => {
...
const icVceTrainrrSaveVisitDates = async (dates: string[]) => {
  const unique = [...new Set(dates)].sort();
  await AsyncStorage.setItem(icVceTrainrrSTORAGE_KEY, JSON.stringify(unique));
};

export const icVceTrainrrRecordAppVisit = async (date = new Date()): Promise<string[]> => {
  const todayKey = formatVisitDateKey(date);
  const dates = await icVceTrainrrLoadVisitDates();

  if (!dates.includes(todayKey)) {
    dates.push(todayKey);
    await icVceTrainrrSaveVisitDates(dates);
  }
